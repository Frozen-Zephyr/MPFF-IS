...
def data_png(file,out):
    '''样本数据分布图'''
    # 读取 CSV
    df = pd.read_csv(file)  # 你的 CSV 文件名
...
def result_data_png(file,out):
    '''结果分布lg柱状图'''
    # 读取CSV
    df = pd.read_csv(file)  # 记得改成你的csv路径
...
def violin(file,out):
    # 读CSV
    df = pd.read_csv(file)  # <- 改成你的路径

    # 画小提琴图
    plt.figure(figsize=(8, 6))
    ax=sns.violinplot(x='Label', y='out', data=df, palette='Set2', cut=0,  inner=None,   )

    ax.collections[0].set_facecolor('lightcoral')  # 红色
...
if __name__ == '__main__':
    path = '/Users/zephyr/Documents/Bioengineering Project/25大创KRASG12D抑制剂筛选预测/result/训练结果文件/泛数据集训练/testset.csv'
    out = '/Users/zephyr/Documents/Bioengineering Project/25大创KRASG12D抑制剂筛选预测/result/训练结果文件/泛数据集训练/step.png'
    while True:
        code = input('mission:')

        if code == 'data_png':
            path='/Users/zephyr/Documents/PycharmProjects/KRASG12D_Inhibitors/Compound_activity_prediction/DataSet/csv/conbine/DUDE.csv'
            out='/Users/zephyr/Documents/PycharmProjects/KRASG12D_Inhibitors/Compound_activity_prediction/DataSet/csv/conbine/DUDE.png'
            data_png(path,out)
            break

        elif code == 'result_png':
            path='/Users/zephyr/Documents/Bioengineering Project/25大创KRASG12D抑制剂筛选预测/result/训练结果文件/模型单蛋白/val.csv'
            out='/Users/zephyr/Documents/Bioengineering Project/25大创KRASG12D抑制剂筛选预测/result/训练结果文件/模型单蛋白/bal.png'
            result_data_png(path,out)
            break

        elif  code == 'dot':
            path='/Users/zephyr/Desktop/log/final_log/kras_test.csv'
            out='/Users/zephyr/Desktop/log/final_log/kras_test.png'
            dot(path,out)
            break

        elif code == 'violin':
            path = '/Users/zephyr/Documents/Bioengineering Project/25大创KRASG12D抑制剂筛选预测/result/训练结果文件/模型蛋白/val.csv'
            out = '/Users/zephyr/Documents/Bioengineering Project/25大创KRASG12D抑制剂筛选预测/result/训练结果文件/模型单蛋白/violin.png'
            violin(path,out)
            break

        elif code == 'training_png':
            input_path = "/Users/zephyr/Desktop/log/layernorm最优/图像/epoch_loss.csv"
            output_path = "/Users/zephyr/Desktop/log/layernorm最优/图像/epoch_loss.png"
            training_png(input_path, output_path)
            break

        elif code == 'kde':
            path='/Users/zephyr/Documents/Bioengineering Project/25大创KRASG12D抑制剂筛选预测/result/训练结果文件/模型单蛋白/val.csv'
            out='/Users/zephyr/Documents/Bioengineering Project/25大创KRASG12D抑制剂筛选预测/result/训练结果文件/模型单蛋白/KDE.png'
            KDE(path,out)
            break
