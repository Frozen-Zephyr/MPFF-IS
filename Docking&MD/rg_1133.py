...
# 定义三个XVG文件路径和对应的标签
file_paths = [
    r"D:\md2\1133\results\gyrate.xvg",
    r"D:\md2\1133\1_5079\results\gyrate.xvg",
    r"D:\md2\1133\2_5406\results\gyrate.xvg",
    r"D:\md2\1133\3_4876\results\gyrate.xvg"
]
...
for file, label, color in zip(file_paths, labels, colors):
    ...
    # 绘制原始数据和平滑曲线
    plt.plot(time, rmsd, color=color, linewidth=1.2, alpha=0.85, label=label)
    #plt.plot(time, rmsd, color=color, alpha=0.7, label=f"{label} ")
    #plt.plot(clean_time, clean_rmsd, color=color, linewidth=0.5, label=f"{label} ")
    #plt.plot(clean_time, smooth_rmsd,color=color, linewidth=2, label=f"{label} (smoothed)")
...
# 添加图表元素
plt.title("Comparison of Rg Profiles", fontsize=14)
plt.xlabel("Time(ps)", fontsize=12)
plt.ylabel("Radius (nm)", fontsize=12)
# 移动图例到图内右上角
