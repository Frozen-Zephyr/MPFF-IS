...
# dataset_train = loading_data(train_file_path,device)
# dataset_valid = loading_data(valid_file_path,device)
# dataset_test = loading_data(test_file_path,device)

# dataset_train, temp_dataset = train_test_split(molecule_data, test_size=0.2, random_state=42)
# dataset_valid, dataset_test = train_test_split(temp_dataset, test_size=0.5, random_state=42)

# loss_trans=BCEWithLogitsLoss()
# loss_trans=loss_trans.to(device)
# mpnn=MPNNPredictorWithProtein(dropout=0.5).to(device)
