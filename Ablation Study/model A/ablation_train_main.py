...
from ablation_predictor import PredictorWithProtein
...
def train_valid(model,
    ...
    for epoch in range(epochs):
        ...
        for i in dataloader_train:          #训练模型
            ligand_feats,protein_feats,label= i  # 解包 tuple
            protein_feats = protein_feats.to(device)
            ligand_feats = ligand_feats.to(device)
            label = label.to(device)
            label = label.view(-1,1)

            output = model(protein_feats, ligand_feats)
        ...
        with torch.no_grad():
            for i in dataloader_valid:
                protein_feats, ligand_feats, label = i  # 解包 tuple
                protein_feats = protein_feats.to(device)
                ligand_feats = ligand_feats.to(device)
                label = label.to(device)
                label = label.view(-1, 1)
...
def Bayesian(loss_trans,
    ...
    writer_by_path = 'ablation_log_by'
    writer_by = SummaryWriter(writer_by_path)
    def objective(trial):       # 贝叶斯优化目标函数
        trial_num = trial.number
        writer_path='log_by/log_by_{}'.format(trial_num)
        writer = SummaryWriter(writer_path)
        model = PredictorWithProtein().to(device)  # 🎯 重新初始化模型

        # model.load_state_dict(torch.load('best_model0.pth', weights_only=False))  # 加载训练好的权重
...
if __name__ == '__main__':
    while True:
        ...
        if code == 'train':
            ...
            model = PredictorWithProtein().to(device)
            # model.load_state_dict(torch.load('best_model0.pth', weights_only=False))
            ...
            model = PredictorWithProtein().to(device)
            model.load_state_dict(torch.load('best_model.pth',weights_only=False) ) # 加载训练好的权重
            model.eval()
