...
class MPNNPredictorWithProtein(nn.Module):
    ...
    def forward(self, graph_feats, node_feats, edge_feats, protein_feats, Ad):
        ...
        node_feats = self.unbatch_node_feats(node_feats, graph_feats, max_nodes)        #（batchsize，Lmax，64）
        node_feats = node_feats.to(device)

        # 蛋白质分支
        protein_feats = self.protein_projector(protein_feats)  # 输出形状: (batch_size,L， 64)
        # 蛋白-化合物特征融合
        # combined_feats = self.cat(node_feats,Ad, protein_feats)
        combined_feats = torch.cat([node_feats, protein_feats], dim=1)
        # 预测活性
